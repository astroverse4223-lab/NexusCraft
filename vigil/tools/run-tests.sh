...
for jar in "$MC" "$BRIG" "$DFU" "$FAST" "$JOML"; do
  [ -n "$jar" ] || { echo "missing a jar; run ./gradlew build first"; exit 1; }
done

CP="$HERE/build/classes/java/main;$MC;$BRIG;$DFU;$FAST;$JOML"

mkdir -p "$HERE/build/testclasses"
"$JAVA_BIN/javac.exe" -nowarn -cp "$CP" -d "$HERE/build/testclasses" "$HERE"/tools/*Test.java

for test in "$HERE"/tools/*Test.java; do
  name=$(basename "$test" .java)
  echo "--- $name ---"
  "$JAVA_BIN/java.exe" -cp "$CP;$HERE/build/testclasses" "dev.nexuscraft.vigil.$name"
done
